...
GAT::GAT(int nfeat, int nhid, int nclass, double dropout, double alpha, int nheads)
    : dropout(dropout), nhid(nhid),alpha(alpha), nheads(nheads), nclass(nclass) {
    for (int i = 0; i < nheads; ++i) {
        attentions.push_back(GatLayer(nfeat, nhid, dropout, alpha, true));
    }
    out_att = GatLayer(nhid * nheads, nclass, dropout, alpha, false);
}

Eigen::MatrixXd GAT::forward(const Eigen::MatrixXd& Xx, const Eigen::MatrixXd& adj){
    // X = X.unaryExpr([this](double x) { return dropNode(x); });
    int el = 1;
    Eigen::MatrixXd X = Xx.unaryExpr([this](double x) { return dropNode(x); });
    int numAttentions = attentions.size();
    std::vector<Eigen::MatrixXd> attentionResults;

    for (int i = 0; i < nheads; ++i) {
        cout<<" Layer n-"<< i+1<<endl;
        Eigen::MatrixXd r = attentions[i].forward(X, adj);
    ...
    void GAT::backward(const Eigen::MatrixXd& X, const Eigen::MatrixXd& adj, const Eigen::MatrixXd& grad_output) {
    ...
    for (int i = nheads - 1; i >= 0; --i) {
        Eigen::MatrixXd grad_x = std::get<2>(attentions[i].backward(X, adj, grad_h));
...
Eigen::VectorXd GAT::evaluate_node(const Eigen::MatrixXd& node_features, const Eigen::MatrixXd& adj) {
    ...
    h = h.unaryExpr([this](double x) { return dropNode(x); });
    for (const auto& att : attentions) {
        for (int i = 0; i < nheads; ++i) {
            h += attentions[i].forward(node_features, adj);
        }
    }
    h = out_att.forward(h, adj);
    h = h.unaryExpr([this](double x) { return dropNode(x); });
...
for (int epoch = 0; epoch < num_epochs; ++epoch) {
    // Matrices pour accumuler les gradients sur l'ensemble du lot
        ...
        //         batch_grad_x.row(i) = grad_x;
        //         batch_grad_adj.row(i) = grad_adj;
        //         // batch_grad_output(i - batch_start) = grad_output;
        //         batch_grad_output(i) = grad_output.mean();  
...
//         : dropout(dropout), alpha(alpha), nheads(nheads), nclass(nclass) {
//         for (int i = 0; i < nheads; ++i) {
//             attentions.push_back(GatLayer(nfeat, nhid, dropout, alpha, true));
//         }
//         out_att = GatLayer(nhid * nheads, nclass, dropout, alpha, false);
...
//         // Backward pass for GAT
//         Eigen::MatrixXd grad_h = grad_output; // Gradients with respect to the output of the final layer
...
//         }
//     }
//      Eigen::VectorXd evaluate_node(const Eigen::VectorXd& node_features, const Eigen::MatrixXd& adj) {
//         // Forward pass through the GAT for a single node
//         Eigen::MatrixXd h = node_features;
...
//                const Eigen::VectorXd& labels, const Eigen::VectorXd& train_mask,const Eigen::VectorXd& val_mask) {
//             for (int i = 0; i < features.rows(); ++i) {
//                 if (train_mask(i) > 0) {
//                     // Extraire les caractéristiques, la matrice d'adjacence et l'étiquette pour cet exemple
//                     Eigen::VectorXd node_features = features.row(i);
//                     Eigen::VectorXd node_labels(nclass); // Supposons une classification binaire
//                     node_labels << 1.0 - labels(i), labels(i);

//                     // Effectuer une passe avant pour obtenir les prédictions
//                     Eigen::VectorXd predictions = evaluate_node(node_features, adjacency_matrix);

//                     // Calculer la perte (negative log-likelihood)
//                     predictions = predictions.array().log();
//                     double loss = -1*node_labels.dot(predictions);

//                     // Effectuer une passe arrière pour calculer les gradients
//                     // backward(node_features, adjacency_matrix, (predictions - node_labels).transpose());

//                     // Mise à jour des paramètres du modèle (par exemple, utiliser un optimiseur comme Adam)
//                     // Vous devrez implémenter cette fonction

//                     // Affichage de la perte pour le suivi
//                 }
//             }
...
//                  const Eigen::VectorXd& labels, const Eigen::VectorXd& train_mask, 
//                  const Eigen::VectorXd& val_mask, int batch_size, int num_epochs) {
//     for (int epoch = 0; epoch < num_epochs; ++epoch) {
//         for (int batch_start = 0; batch_start < features.rows(); batch_start += batch_size) {
//             int b = batch_start + batch_size;
...
//                     const Eigen::VectorXd& labels, const Eigen::VectorXd& eval_mask) {
//         int correct_predictions = 0;
//         int total_examples = 0;
